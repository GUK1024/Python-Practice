...
a[1] = 4
print(a)
print(b) # a[1]의 값만 바꿨는데 b도 같은 이유는 a와 b 둘다 같은 주소를 가지고 있기 때문 

print(id(a))
print(id(b)) #id로 메모리 주소를 보면 같은걸 알수있다.

print(a is b) #같은 메모리 주소를 갖고 있는지 불 자료형으로 확인할 수 있다.


a = [1,2,3]
b = a[:] # b는 바꾸고 싶지 않으면 리스트 전체의 벨류값을 전해주면 된다 (b의 새로운 리스트가 생김)
a[1] = 4
print(a)
print(b)

from copy import copy #Copy모듈로 복사하는 기능을 가져와도 가능.
a = [1,2,3]
b = copy(a)
a[1] = 4
print(a)
...
tmp = b # tmp라는 임시 변수 생성 (6의 벨류값을 보존해주는 변수)
b = a # b의 메모리주소를 a로 변경
a = tmp # a의 메모리주소를 tmp로 변경
print(a)
print(b)


#벨류값을 각각 바꾸고 싶을때 *파이썬에서는 이 방법이 있음
